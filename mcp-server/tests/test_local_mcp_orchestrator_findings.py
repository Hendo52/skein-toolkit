...
mcp-server/local-mcp.py: `_extract_step_finding`, `_format_findings_block`, the
`findings` field on `_new_orchestrator_state`, and `_build_step_dispatch_body`
prepending the "Prior step findings" block.

Run with: .venv\\Scripts\\python.exe mcp-server\\tests\\test_local_mcp_orchestrator_findings.py
